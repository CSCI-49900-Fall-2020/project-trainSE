...
const auth = async (req, res, next) => {
  try {
    // The client sends over a header with field x-auth-token
    ...
    // That field will contain the decrypted id from the verified decoded token
    req.user = decoded;

    // Middleware ends, continue to execute the next function
    // Next function in the route syntax most likely the (req, res) part
    next();
  } catch (err) {
